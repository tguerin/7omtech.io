...
Did I want to become a doctor, raise sheep in the Pyrenees, or become a blacksmith forging magical swords and hunting monsters?  
...
A week after releasing my app, one of my ex-colleagues reached out to catch up. During our chat, he told me he was about to work on a Flutter app. The subject was confidential, but the real pain point was that the company wasn’t in Paris but in… (let me build some suspense)… **Bordeaux!**  
...
Then we started implementing the game itself. We decided not to use **Flame** since it wasn’t really a video game, but instead went for an *app-like game toolkit* (link).  
...
For example:  
- Our `KoCenterWidget`, which displays the bik KO animation when eliminating a player in a PKO tournament, listens for animations on `KoTarget`.  
...
The release date was finally set for **December 9**,  right in the middle of my holidays.  
...
In less than **two weeks**, all of these features were shipped.  
